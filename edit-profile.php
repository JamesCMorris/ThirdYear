        ...
        <section>
          <?php
            if (isset($_SESSION['userId'])) {
                ...
                $sql = "SELECT fullName, postCode, email, position, user_bio FROM users WHERE userId = '" . $_SESSION['userId'] . "'";
                $result = mysqli_query($conn, $sql);

                echo '<p class="login-status">You are logged in as ' . $_SESSION['userName'] . '!</p>
                <h2>Edit your profile here!</h2>
                <form action="/php/includes/edit-user-info.php" method="POST">
                <input type="text" name="fullname" placeholder="Enter New Full Name..">
                <input type="text" name="postcode" placeholder="Enter New Postcode..">
                <input type="text" name="email" placeholder="Enter New Email..">
                <input type="text" name="position" placeholder="Enter New Position..">
                <textarea name="userbio" placeholder="Enter New User Bio.."></textarea>
                </br>
                <button type="submit" name="edit-info" class="submitbutton">Submit Edits</button>
